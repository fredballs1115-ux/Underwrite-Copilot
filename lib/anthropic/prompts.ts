...
Overlay the down-cycle discipline the industry keeps re-learning in every crash: leasing and rent assumptions must come from today's market, not from what the pro forma needs to pencil; when the market is softening, treat this quarter's worst case as a candidate for next quarter's base case; judge the deal on the cash it actually throws off, not on appraisal values or exit hopes — a return that lives mostly in the residual is a warning, not a plan; and "other buyers are circling" is never underwriting support. Where an assumption exists only to make the deal work on paper, say exactly that.
