...
First say what KIND of deal this is — \`strategy.kind\`: "stabilized" (an operating asset bought for its in-place income), "value_add" (in-place income plus a renovation or repositioning program), "lease_up" (largely vacant space still to be leased), "conversion" (a change of use — office to residential and the like — with construction and downtime before any stabilized income), "development" (ground-up or to-be-built), or "unknown" only if the OM truly does not say. Give \`strategy.summary\` (one sentence, the plan in the OM's own terms; "" if it states none), \`strategy.capitalBudget\` (the renovation or construction budget as stated, hard and soft; "" if none) and \`strategy.timeline\` (construction, downtime and lease-up timing to stabilization as stated; "" if none). For a conversion, development or lease-up, also capture as metrics — each with its page — the total project cost or all-in basis, the construction budget, the construction period, and the year the plan stabilizes.
