...
IF THE OM OFFERS THE SELLER'S LOAN FOR ASSUMPTION — debt in place that a buyer may take over, never a quote for new financing or the OM's own proposed loan — label each of its terms the OM states exactly: "Assumable loan balance" (today's unpaid balance), "Assumable loan rate", "Assumable loan maturity" (as written, with its month), "Assumable loan amortization" ("Interest-only", or the years as stated), "Assumable loan debt service" (the annual payment) and "Assumption fee" (as stated). A term the OM does not state is left out, never derived.
...
The deal sits in a market the site tracks, and the metro's own published figures follow — each dated, each with its publisher — with the debt market's after them. Where one answers an OM assumption, check the assumption against the FIGURE and cite the figure with its date in the note: rent growth against the rent index and the asking-rent change (on a commercial building, against the national index of rents its kind of lessor charges, said as the nation's), occupancy and vacancy against the metro's rental vacancy (and its margin — a move inside the margin is noise), lease-up, absorption and occupancy claims on a commercial building against the metro's payrolls in the sector that fills its kind (the line names the sector — professional and business services for an office, transportation and warehousing for a warehouse, retail trade for a store, leisure and hospitality for a hotel, education and health for a clinic), supply claims against the year of permits (on rental housing, against the units in buildings of two or more within it — the pipeline an apartment competes with, said as the total less the single-family series), an exit story against the for-sale market's direction and its hotness rank, the exit cap and the debt assumptions against the 10-year Treasury and what banks say about their standards for this kind of loan, an exit value that leans on prices rising against the national commercial property price index (a trailing year, the nation's — a sale price the index has not reached is a forecast, not a comparable), and the insurance line against the national index of commercial property insurance premiums (a memorandum carries the seller's expiring policy; the index says how far a new owner's quote has moved). State a metro figure as the metro's and a state figure as the state's (a deal outside the tracked metros is handed its state's figures, and the block says so), never as the submarket's or the building's, and keep the typical range in \`typicalRange\` as the norm the figure is read beside — a figure narrows the range, it does not replace the OM's own numbers. Where no figure answers, the typical range stands on its own.
